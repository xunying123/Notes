...
MacOS用户请点击屏幕左上角苹果图标-关于本机，查看处理器一栏，若是Intel芯片请下载 `jdk-8u411-macosx-x64.dmg`，若是Apple芯片请下载 `jdk-8u411-macosx-aarch64.dmg`
...
然后**点击你需要安装的游戏版本**。本社团服务器大部分均可用 `1.20.1` 进入。
...
- 服务器类型
  ...
  对于SJMC服务器，除方块交大展示服外的服务器均需要皮肤站登录
...
- **登录**
  ...
  游戏安装完成后，在 `启动`页面，点击 `版本设置`，在 `设置`中找到 `服务器`，登录方式选择 `第三方登录：Authlib Injector 或 LittleSkin`，认证服务器填写https://skin.mc.sjtu.cn/api/yggdrasil，非上海交通大学的同学请输入https://skin.mualliance.ltd/api/yggdrasil，注册链接不用填写，填写完毕后直接返回即可。
